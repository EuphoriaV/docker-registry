...
public class UploadBlobChunkOperation {
    ...
    public long activate(String name, UUID id, Resource body, String range, long contentLength) {
        if (contentLength < 0) {
            throw new InvalidRequestException("content-length header is required", ErrorResponse.ErrorCode.SIZE_INVALID);
        }

        return lockService.tryInLock(id.toString(), () -> uploadChunk(name, id, body, range, contentLength));
    }
